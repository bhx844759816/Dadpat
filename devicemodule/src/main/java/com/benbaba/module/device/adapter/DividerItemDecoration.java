...
public class DividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void drawVertical(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            final View child_next = parent.getChildAt(i + 1);
            int type = (int) child.getTag();
            int type_next = 0;
            if (child_next != null) {
                type_next = (int) child_next.getTag();
            }
            final RecyclerView.LayoutParams params = (RecyclerView.LayoutParams) child.getLayoutParams();
            final int top = child.getBottom() + params.bottomMargin;
            final int bottom = top + mDividerHeight;
            if (mDivider != null) {
                mDivider.setBounds(type == 1 && type_next == 1 ? left + 40 : left, top, right, bottom);
                mDivider.draw(c);
            }
            if (mPaint != null) {
                c.drawRect(type == 1 && type_next == 1 ? left + 40 : left, top, right, bottom, mPaint);
            }
        }
    }
